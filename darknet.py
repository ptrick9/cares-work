...
lib = CDLL("/home/nvidia/yolo/darknet/libdarknet.so", RTLD_GLOBAL)
...
def gen_files():
    basePath = '/home/nvidia/data_training_v3/data_training/'
...
'''
    ...
    for i, folder in enumerate(x):
        results = []
        print("%f %s" % (float(i/len(x)), folder[1]))

        for file in folder[1:]:
            r = detect(net, meta, "%s.jpg" % file)
            results.append([file, r])
            #print(results)


        f = open("python_res/%s" % folder[0], 'wb')
